  ...
  <div class="grid-wrapper">
    <div class="cluster-section">
      <div class="cluster-header">
        <span>CLUSTER MAP — {{ stats.total }} NODES ({{ stats.cats }} CAT / {{ stats.kw }} KW / {{ stats.sites }} SITES)</span>
        <div class="legend">
      ...
      <div v-for="a in filteredArticles" :key="a.id" class="card" @click="openArticle(a)">
        ...
        <div class="card-top">
          <div class="card-source">
            <span class="src">{{ a.source }}</span><span class="author">| {{ a.author }}</span><span v-if="a.flagged" class="flag-badge">FLAGGED</span>
          </div>
        ...
        <div class="card-tags">
          <span v-for="tag in a.tags" :key="tag.t" class="tag" :style="tag.s" @click.stop="emit('search-tag', tag.t)">{{ tag.t }}</span>
        </div>
        <div class="card-foot">
          <div class="foot-labels"><span>RELEVANCE</span><span>WORDS</span><span class="ml-auto">COLLECTED</span></div>
          <div class="foot-values">
            <div class="relevance"><div class="bar"><div :style="{ width: a.relevance + '%' }"></div></div><span>{{ a.relevance }}</span></div>
            <span>{{ (a.words || 0).toLocaleString() }}</span><span class="collected">{{ a.collected }}</span>
          </div>
          <div class="collection-label" :style="{ color: a.color }"><div class="sq" :style="{ background: a.color }"></div>{{ a.collection }}</div>
        </div>
      </div>
    </div>
    ...
    <div v-if="selected" class="modal" @click.self="selected = null">
      <div class="modal-content">
        <div class="modal-head">
          <div class="card-source"><span class="src">{{ selected.source }}</span><span class="author">| {{ selected.author }}</span></div>
          <span class="close" @click="selected = null">✕</span>
        </div>
        <h1 class="modal-title">{{ selected.title }}</h1>
        <p class="modal-summary">{{ selected.summary }}</p>
        <div class="modal-meta">RELEVANCE: <b>{{ selected.relevance }}</b> WORDS: <b>{{ selected.words }}</b><span class="ml-auto muted">{{ selected.collected }}</span></div>
        <div class="modal-actions">
          <button class="btn-black large" :disabled="!getArticleUrl(selected)" @click.stop="openFullArticle">{{ getArticleUrl(selected) ? "OPEN FULL ARTICLE ↗" : "NO LINK AVAILABLE" }}</button><button class="btn-outline">FLAG</button>
        </div>
      </div>
    </div>
  </div>
...
async function getArticles() {
  const urls = ["/api/items", "http://127.0.0.1:5000/api/items"];
  let lastErr = null;
  ...
  throw lastErr || new Error("Failed to fetch articles");
}
...
<style scoped>
.grid-wrapper { flex: 1; background: #f7f7f5; padding: 16px; }
.cluster-section { background: #fefefd; border: 1.5px solid #111; margin-bottom: 16px; }
.cluster-header { display: flex; justify-content: space-between; align-items: center; padding: 10px 14px; border-bottom: 1.5px solid #111; flex-wrap: wrap; gap: 8px; font-family: "IBM Plex Mono", monospace; font-size: 9px; font-weight: 700; }
.legend { display: flex; gap: 12px; font-size: 8px; font-weight: 400; }
.legend i { display: inline-block; width: 8px; height: 8px; margin-right: 4px; }
.graph-canvas { width: 100%; height: 460px; }
.grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(340px, 1fr)); gap: 12px; align-content: start; }
.card { background: #fefefd; padding: 16px 16px 12px; display: flex; flex-direction: column; min-height: 260px; position: relative; overflow: hidden; cursor: pointer; transition: all 0.18s ease; border: 1px solid #e5e2de; }
.card:hover { transform: translateY(-2px); box-shadow: 0 8px 24px rgba(0, 0, 0, 0.12); z-index: 2; }
.flag-line { position: absolute; top: 0; left: 0; width: 3px; height: 100%; background: #ff5a1f; }
.card-top { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 12px; flex-shrink: 0; }
.card-source { display: flex; gap: 8px; align-items: center; font-family: "IBM Plex Mono", monospace; font-size: 8px; letter-spacing: 0.06em; text-transform: uppercase; }
.src { font-weight: 800; color: #111; font-size: 9px; }
.author { color: #999; }
.flag-badge { background: #ff5a1f; color: #111; padding: 1px 4px; font-weight: 700; font-size: 7px; }
.card-badges { display: flex; gap: 4px; align-items: center; }
.badge { font-family: "IBM Plex Mono", monospace; font-size: 7px; padding: 3px 6px; }
.open-btn { font-size: 11px; color: #999; border: 1px solid #e5e2de; width: 18px; height: 18px; display: flex; align-items: center; justify-content: center; }
.card-title { font-size: 13.5px; font-weight: 700; line-height: 1.28; min-height: 51.8px; max-height: 51.8px; display: -webkit-box; line-clamp: 3; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden; margin-bottom: 10px; flex-shrink: 0; }
.card-summary { font-size: 11px; line-height: 1.5; color: #666; min-height: 49.5px; max-height: 49.5px; display: -webkit-box; line-clamp: 3; -webkit-line-clamp: 3; -webkit-box-orient: vertical; overflow: hidden; margin-bottom: 14px; flex-shrink: 0; }
.card-tags { display: flex; gap: 6px; flex-wrap: wrap; min-height: 22px; margin-bottom: 16px; flex-shrink: 0; }
.tag { font-family: "IBM Plex Mono", monospace; font-size: 8px; padding: 4px 8px; cursor: pointer; font-weight: 800; letter-spacing: 0.04em; text-transform: uppercase; }
.card-foot { margin-top: auto; flex-shrink: 0; }
.foot-labels { display: flex; justify-content: space-between; font-family: "IBM Plex Mono", monospace; font-size: 7px; color: #aaa; margin-bottom: 4px; text-transform: uppercase; gap: 12px; }
.foot-values { display: flex; justify-content: space-between; align-items: center; font-family: "IBM Plex Mono", monospace; font-size: 10px; }
.relevance { display: flex; align-items: center; gap: 5px; }
.bar { width: 36px; height: 3px; background: #e5e2de; border-radius: 2px; overflow: hidden; }
.bar div { height: 100%; background: #111; }
.collected { color: #999; font-size: 8px; }
.collection-label { margin-top: 10px; display: flex; align-items: center; gap: 5px; font-family: "IBM Plex Mono", monospace; font-size: 8px; font-weight: 600; }
.sq { width: 6px; height: 6px; }
.modal { position: fixed; inset: 0; background: rgba(0, 0, 0, 0.6); display: flex; z-index: 200; align-items: center; justify-content: center; padding: 20px; }
.modal-content { background: #fefefd; width: 100%; max-width: 640px; max-height: 90vh; overflow: auto; border: 1.5px solid #111; box-shadow: 0 20px 60px rgba(0, 0, 0, 0.3); }
.modal-head { padding: 24px 24px 0; display: flex; justify-content: space-between; align-items: center; font-family: "IBM Plex Mono", monospace; font-size: 9px; text-transform: uppercase; }
.close { cursor: pointer; font-size: 14px; border: 1px solid #e5e2de; width: 28px; height: 28px; display: flex; align-items: center; justify-content: center; }
.modal-title { font-size: 22px; font-weight: 800; line-height: 1.25; margin: 16px 24px 12px; letter-spacing: -0.02em; }
.modal-summary { font-size: 14px; line-height: 1.6; color: #444; margin: 0 24px; }
.modal-meta { padding: 16px 24px; background: #faf8f5; display: flex; gap: 16px; font-family: "IBM Plex Mono", monospace; font-size: 10px; margin-top: 18px; flex-wrap: wrap; }
.modal-actions { padding: 16px 24px; display: flex; gap: 10px; }
.btn-black.large { flex: 1; height: 42px; background: #111; color: #fff; font-family: "IBM Plex Mono", monospace; font-size: 11px; font-weight: 600; border: none; cursor: pointer; }
.btn-outline { height: 42px; padding: 0 18px; background: #fff; border: 1.5px solid #111; font-family: "IBM Plex Mono", monospace; font-size: 11px; font-weight: 600; cursor: pointer; }
.ml-auto { margin-left: auto; }
.muted { color: #aaa; }
.loading-state, .error-state { font-family: "IBM Plex Mono", monospace; font-size: 11px; padding: 20px; }
</style><template>
  <div class="grid-wrapper">
    <div class="cluster-section">
      <div class="cluster-header">
