      ...
      <div class="filter-bar">
        ...
        <div class="filter-section">
          <div class="search-input">
            <span>⌕</span><input :value="search" @input="emit('update:search', ($event.target as HTMLInputElement).value)" placeholder="Search..." />
          </div>
        </div>
        <div class="filter-section">
          <label class="label">SORT BY</label><select :value="sort" @change="emit('update:sort', ($event.target as HTMLSelectElement).value)">
...
function toggleStatus(val: string) {
  const n = props.status.includes(val) ? props.status.filter((v) => v !== val) : [...props.status, val];
  emit("update:status", n);
}
function togglePriority(val: string) {
  const n = props.priority.includes(val) ? props.priority.filter((v) => v !== val) : [...props.priority, val];
...
.search-input { display: flex; align-items: center; gap: 8px; border: 1px solid #e5e2de; background: #fff; padding: 8px 10px; }
.search-input input { border: none; outline: none; font-size: 12px; font-family: inherit; width: 100%; background: transparent; }
...
.cat-item { padding: 7px 8px; font-size: 11px; cursor: pointer; color: #666; border-radius: 2px; }
.cat-item:hover { background: #fff; }
.cat-item.active { background: #111; color: #fff; font-weight: 600; }
.check { display: flex; gap: 8px; align-items: center; font-size: 11px; padding: 6px 0; cursor: pointer; color: #333; }
.filter-actions { padding: 16px; }
.btn-clear-full { width: 100%; height: 38px; border: 1.5px solid #111; background: #fff; font-family: inherit; font-size: 10px; font-weight: 700; letter-spacing: 0.06em; cursor: pointer; }
