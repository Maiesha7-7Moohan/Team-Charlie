import { Mn as effectScope, U as computed, Wn as reactive, gn as watch, xt as inject } from "./vue.runtime.esm-bundler-CR3Nwe7D.js";
...
//#endregion
//#region node_modules/vuex/node_modules/@vue/devtools-api/lib/esm/const.js
var HOOK_SETUP = "devtools-plugin:setup";
var HOOK_PLUGIN_SETTINGS_SET = "plugin:settings:set";
//#endregion
//#region node_modules/vuex/node_modules/@vue/devtools-api/lib/esm/time.js
var supported;
var perf;
...
function resetStoreState(store, state, hot) {
	...
	if (oldState) {
		if (hot) store._withCommit(function() {
			oldState.data = null;
		});
	}
...
function installModule(store, rootState, path, module, hot) {
	...
	module.forEachAction(function(action, key) {
		registerAction(store, action.root ? key : namespace + key, action.handler || action, local);
	});
...
function makeLocalContext(store, namespace, path) {
	...
	Object.defineProperties(local, {
		getters: { get: noNamespace ? function() {
...
function registerMutation(store, type, handler, local) {
	(store._mutations[type] || (store._mutations[type] = [])).push(function wrappedMutationHandler(payload) {
		handler.call(store, local.state, payload);
	});
}
function registerAction(store, type, handler, local) {
	(store._actions[type] || (store._actions[type] = [])).push(function wrappedActionHandler(payload) {
...
function addDevtools(app, store) {
	setupDevtoolsPlugin({
		...
		api.on.getInspectorTree(function(payload) {
			if (payload.app === app && payload.inspectorId === INSPECTOR_ID) if (payload.filter) {
				var nodes = [];
				flattenStoreForInspectorTree(nodes, store._modules.root, payload.filter, "");
				payload.rootNodes = nodes;
			} else payload.rootNodes = [formatStoreForInspectorTree(store._modules.root, "")];
		});
...
function formatStoreForInspectorState(module, getters, path) {
	...
	var storeState = { state: Object.keys(module.state).map(function(key) {
		return {
			key,
			editable: true,
...
ModuleCollection.prototype.register = function register(path, rawModule, runtime) {
	...
	else this.get(path.slice(0, -1)).addChild(path[path.length - 1], newModule);
...
Store.prototype.install = function install(app, injectKey) {
	...
	if (this._devtools !== void 0 ? this._devtools : true) addDevtools(app, this);
};
...
Store.prototype.subscribeAction = function subscribeAction(fn, options) {
	return genericSubscribe(typeof fn === "function" ? { before: fn } : fn, this._actionSubscribers, options);
};
